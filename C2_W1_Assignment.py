...
def get_count(word_l):
    ...
    ### START CODE HERE 
    for unique_word in set(word_l):
        word_count_dict[unique_word] = word_l.count(unique_word)
...
def get_probs(word_count_dict):
    ...
    ### START CODE HERE ###
    ## get all values, add them to find total number of words
    total_n_words = sum(list(word_count_dict.values()))
    ## get the frequency of the word
    for key in word_count_dict.keys():
        probs[key] = float(word_count_dict[key]/total_n_words)
...
# delete_l = ['ans', 'cns', 'cas', 'can']
# ```
# * => Because we are checking for empty R condition inside for-loop
...
def edit_one_letter(word, allow_switches = True):
    ...
    ### START CODE HERE ###
    l = insert_letter(word, False) + delete_letter(word, False) + replace_letter(word, False)
    if  allow_switches:
        l.extend(switch_letter(word, False))
...
def edit_two_letters(word, allow_switches = True):
    ...
    edit_two = []
    for each_word in edit_one:
        edit_two.extend(edit_one_letter(each_word, allow_switches))
    edit_two_set = set(edit_two)
...
print(val2)
val3 = True or [] or [] or ["least","of","all"]
val4 = False or [] or [] or ["least","of","all"] or "Word"
print(val3)
print(val4)
val5 = [] or [] or "Word"
print(val5)
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    ### START CODE HERE ###
    suggestions = set([word]).intersection(vocab) or edit_one_letter(word).intersection(vocab)             or edit_two_letters(word).intersection(vocab) or set([word])
    #print(suggestions)
    
    ## step 2
    best_words = dict()
    for each in suggestions:
        best_words[each] = probs.get(each, 0.0)
    
    ## step 3
    #n_all = sorted(best_words.items(), key=itemgetter(1), reverse=True)
    #n_all = sorted(best_words, key=best_words.get, reverse=True)
    #print(best_words)
    #n_best = n_all[:n+1]
    #n_best = {w: probs.get(w, 0) for w in n_best}
    #print(type(n_best))
    #print(n_best[0])
...
for i, word_prob in enumerate(tmp_corrections):
    ...
    #print("word {", i, "}: {", word_prob[0], "}, probability {", word_prob[1])
