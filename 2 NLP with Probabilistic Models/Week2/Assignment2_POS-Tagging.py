...
def create_transition_matrix(alpha, tag_counts, transition_counts):
    ...
    for i in range(num_tags):
        ...
        for j in range(num_tags):
            ...
            A[i,j] = (alpha + count)/(count_prev_tag + alpha*num_tags)
...
def create_emission_matrix(alpha, tag_counts, emission_counts, vocab):
    ...
    #stringTags = [str(i) for i in list(tag_counts.keys())]
    all_tags = sorted(tag_counts.keys())
    #all_tags = sorted(stringTags)
...
def initialize(states, tag_counts, A, B, corpus, vocab):
    ...
    for i in range(num_tags): # complete this line
        ...
        if not A[s_idx, i]: # complete this line
        ...
        # For all other cases when transition from start token to POS tag i is non-zero:
            ...
            best_probs[i,0] = np.log(𝐴[s_idx, i]) + np.log(𝐵[i, vocab[𝑐𝑜𝑟𝑝𝑢𝑠[0]]])
